...
object DFSIOSparkADLS {
  ...
  def main(args: Array[String]) {
    ...
    if (args.length < 8) {
      System.err.println("Usage: DFSIOSpark number_of_files file_size_in_bytes file_io_location buffer_size read|write credential clientid refreshurl")
      System.exit(1)
    }
    ...
    val credential = args(5)
    val clientId = args(6)
    val refreshUrl = args(7)
    ...
    val configSerDer = new ConfigSerDeser(sc.hadoopConfiguration)
